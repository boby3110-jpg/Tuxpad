...
未保存のタブが何枚も溜まったときは、**「ファイル → すべて保存」**で
**そのウィンドウの変更されたタブだけ**をまとめて保存できます。1 枚ずつ
選んで `Ctrl+S` を押す必要はありません。保存し終えると、ステータスバーに
「3 件のタブを保存しました」のように件数が出ます。

- **ショートカットキーは割り当てていません**（メニューからのみ）。押し
  間違いで全タブが書き込まれると取り返しがつかないためです。
- **変更の無いタブには書き込みません**（更新日時も動きません。下の
  [変更が無いときの上書き保存](#変更が無いときの上書き保存)と同じ考え方です）。
- **他のウィンドウのタブには手を出しません。**
- 保存先の決まっていない新規タブ（無題）が混ざっていると、そのタブの分だけ
  保存先を尋ねます。**そこでキャンセルしても残りのタブは保存されます**
  （「このタブは今は名前を決めたくない」という意味に取ります）。
- 保存できなかったタブがあるときは、件数のあとに「（1 件は保存していません）」
  が付き、**その最初の 1 枚を表示したまま**にします。
...
確認ダイアログのボタンは、**`Alt` を押さずにキー 1 つで**選べます。

| ダイアログ | キー |
| --- | --- |
| 保存の確認 | `S` 保存 / `N` 破棄 / `C` キャンセル |
| はい・いいえの確認（再読み込み・更新の確認など） | `Y` はい / `N` いいえ |

`Alt` を押しながらの従来のやり方（`Alt+S` など）もそのまま使えます。
これらのキーはダイアログを開いている間だけ効き、本文の編集には影響しません。
...
| ヒットなしのときに音を鳴らす | 検索・置換で 1 件も見つからなかったときに音で知らせる（既定は ON。下記「ヒットなしのときの音」） |
...
検索（`Ctrl+F`）・置換（`Ctrl+H`）で**1 件も見つからなかったとき**、
パネルの件数欄に「見つかりません」と出すのに加えて、**短い警告音**を
鳴らします（お使いのデスクトップ環境の標準の警告音です。音声ファイルの
用意も追加の設定も要りません）。

鳴るのは「自分で検索・置換を実行したとき」だけです。

- 鳴る: 検索欄で `Enter` /「検索」ボタン、「置換」ボタン、「全置換」ボタン
- 鳴らない: 検索語が空のとき、ヒットが 1 件以上あるとき、
  本文を編集したことによる**自動の検索し直し**（打っている最中に
  鳴り続けないように）

不要なら **`設定 → ヒットなしのときに音を鳴らす`** を外してください。
この設定は次回起動時にも引き継がれ、開いている全ウィンドウで共通です。

> デスクトップ環境側で警告音そのものが無効になっていると鳴りません
> （KDE なら「システム設定 → 音と通知」などをご確認ください）。
